...
if (isset($_POST['plus'])) {
    if(count($formateurs)>$_SESSION['compteur']){
        $_SESSION['compteur']++;
    }
    else{
        $_SESSION["errorNBformateur"]="vous ne pouvez pas rajouter de formateur, il n'y plus de formateurs disponnible";
    }
}
if (isset($_POST['moins']) && $_SESSION['compteur'] > 1) {
    $_SESSION['compteur']--;
}
...
?>
 <div class="form-box">
    ...
    <h3>Créeer une formation de personnalisé :</h3>
        <div class="mb-3">
        ...
        <?php
            for ($i = 0; $i < $_SESSION["compteur"]; $i++) { ?>
                <div class="mb-3 d-flex align-items-end gap-2">
                    ...
                    <?php if ($i == $_SESSION["compteur"] - 1) { ?>
                        <button name="moins" class="btn btn-secondary rond">−</button>
                        <button name="plus" class="btn btn-secondary rond">+</button>
                    <?php } ?>

                </div>
            <?php
                if(isset($_SESSION["errorNBformateur"])){
                    echo '<div class="errormessages">'.$_SESSION["errorNBformateur"].'</div>';
                }
            } ?>
        </div>
